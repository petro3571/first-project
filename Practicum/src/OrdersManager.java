...
public class OrdersManager {
    ...
    String getMaxOrderCustomerName() {
        ...
        for (String names: customersOrders.keySet()){
            ArrayList <Double> orders = customersOrders.get(names);
            double sum = 0;
    ...
    void removeUnprofitableOrders() {
        ...
        for (String name: names){
            if (customersOrders.containsKey(name)) {
                customersOrders.remove(name);// удалите из хеш-таблицы тех, чьи расходы строго меньше 5000
